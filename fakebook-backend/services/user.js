...
module.exports = (app, db) => {
  ...
  app.get(
    ...
    async function(req, res) {
      ...
      if (
        requestFromUser &&
        requestFromUser.id == friendId &&
        requestFromUser.status == "request"
      ) {
        const user = await getUserInfo(requestFromUser.id);
        res.status(200).send({ ...user, statusName: "รอคำตอบรับจากคุณ" });
      } else if (
        requestToUser &&
        requestToUser.id == friendId &&
        requestToUser.status == "request"
      ) {
